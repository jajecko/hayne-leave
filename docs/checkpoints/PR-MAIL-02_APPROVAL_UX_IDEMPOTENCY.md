...
Production smoke after MAIL-01 revealed three concrete issues:
...
- Manager-facing new-request mail now includes `Reason` as `Uzasadnienie`.
- The manager CTA now opens `/requests/review/{id}` instead of the generic `/leaves/requests/{id}` view.
- Cancellation-review mail uses the same manager decision route.

### Manager decision surface
- Add an explicit `requests/review/{id}` route and controller action.
...
- Rejection uses a comment form and respects `mandatory_comment_on_reject`.
...
- PHP lint for `requests/review.php` and `hayne_mail_helper.php`;
